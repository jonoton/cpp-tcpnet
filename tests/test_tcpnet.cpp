...
TEST(TcpNetTest, ClientInitiatesConnectionNonBlocking) {
  ...
  // Since connect is non-blocking, it returns a valid session ID and initiates
...
TEST(TcpNetRegressionTest, LoggerThreadSafety) {
  ...
  std::thread setter([&stop]() {
    for (int i = 0; !stop.load(); ++i) {
      if (i % 2 == 0) {
        cpptcpnet::SetLogger(
            [](cpptcpnet::LogSeverity, const std::string&, const std::string&) {
              // no-op logger
            });
      } else {
...
TEST(TcpNetRegressionTest, ClientSetErrorHandlerExists) {
  ...
  EXPECT_NO_THROW(client.SetErrorHandler(
      [&error_fired](int, const std::string&) { error_fired.store(true); }));
...
TEST(TcpNetRegressionTest, ClientConnectAndDisconnectEvents) {
  ...
  worker.AddSubscription<cpptcpnet::ConnectionEvent>(
      sub, [&](const cpptcpnet::ConnectionEvent& e) {
...
TEST(TcpNetRegressionTest, LargePayloadSendReceive) {
  ...
  listener.SetDataHandler(
      [&total_received](uint64_t, const std::vector<uint8_t>& data) {
  ...
  auto sub = client.GetEventBroker().Subscribe<cpptcpnet::ConnectionEvent>(
      "state_events");
  worker.AddSubscription<cpptcpnet::ConnectionEvent>(
      sub, [&client_connected](const cpptcpnet::ConnectionEvent& e) {
        if (e.state == cpptcpnet::ConnectionState::Connected) {
...
TEST(TcpNetRegressionTest, ListenerSetErrorHandlerExists) {
  ...
  EXPECT_NO_THROW(listener.SetErrorHandler(
      [&error_fired](int, const std::string&) { error_fired.store(true); }));

  listener.Stop();
}

// SessionSafetyAndReUsePrevention: verifies that we can fetch unique session
// IDs and that sending with an invalid session ID is safely rejected.
...
TEST(TcpNetMetricsTest, AtomicStatsVerification) {
  ...
  listener.SetDataHandler(
      [&received_bytes](uint64_t, const std::vector<uint8_t>& data) {
...
TEST(TcpNetMetricsTest, PubSubAndThroughputTracker) {
  ...
  cpptcpnet::ThroughputTracker listener_tracker(listener.GetEventBroker(),
                                                std::chrono::milliseconds(500));

  cpptcpnet::TcpClient client;
  // Create throughput tracker for client
  cpptcpnet::ThroughputTracker client_tracker(client.GetEventBroker(),
                                              std::chrono::milliseconds(500));
...
TEST(TcpNetSslTest, ClientConnectsToSslServer) {
  ...
  // Verify cert files exist before starting SSL to prevent crashes or silent
  // failures
  {
  ...
  std::atomic<size_t> received_bytes{0};
  listener.SetDataHandler(
      [&received_bytes, &listener](uint64_t session_id,
  ...
  client.SetDataHandler(
      [&client_received_bytes](uint64_t, const std::vector<uint8_t>& data) {
  ...
  for (int i = 0; i < 200 && (received_bytes.load() < 5 ||
                              client_received_bytes.load() < 5);
       ++i) {
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }
...
TEST(TcpNetConfigTest, GettersAndSetters) {
  ...
  client.SetAutoReconnect(true, std::chrono::milliseconds(100),
                          std::chrono::milliseconds(500));
...
TEST(TcpNetConfigTest, ClientAutoReconnect) {
  ...
  cpptcpnet::TcpClient client;
  client.SetAutoReconnect(true, std::chrono::milliseconds(50),
...
TEST(TcpNetFeatureTest, IPv6ConnectAndPeerAddress) {
  ...
  cpptcpnet::PeerAddress listener_peer =
      listener.GetPeerAddress(listener_session_id);
...
TEST(TcpNetZeroCopyTest, SharedPtrSendVector) {
  ...
  auto shared_vec = std::make_shared<const std::vector<uint8_t>>(
      std::vector<uint8_t>{'s', 'h', 'a', 'r', 'e', 'd', 'v', 'e', 'c'});
  EXPECT_TRUE(client.Send(shared_vec));
  EXPECT_EQ(shared_vec.use_count(),
            2);  // 1 in tests, 1 in client outbound queue
...
TEST(TcpNetZeroCopyTest, SharedPtrSendString) {
  ...
  EXPECT_EQ(shared_str.use_count(),
            2);  // 1 in tests, 1 in client outbound queue
...
TEST(TcpNetConfigTest, SetAndGetProfiles) {
  ...
  auto listener_updated_prof =
      listener.GetConnectionProfile(listener_session_id);
...
TEST(TcpNetRegressionTest, SessionAffinitySerialExecution) {
  ...
  EXPECT_FALSE(concurrent_exec.load())
      << "Callbacks were executed concurrently for the same session!";
