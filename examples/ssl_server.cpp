...
int main() {
    // Under testing guidelines, listen on localhost (127.0.0.1) instead of 0.0.0.0
    ...
    worker.AddSubscription<cpptcpnet::ConnectionEvent>(sub, [](const cpptcpnet::ConnectionEvent& event) {
        if (event.state == cpptcpnet::ConnectionState::Connected) {
            std::cout << "Client connected over SSL/TLS: " << event.session_id << std::endl;
        } else {
    ...
    try {
        server.Start();
        std::cout << "SSL Server running on 127.0.0.1:8443. Press Enter to stop." << std::endl;
        std::cin.get();
        server.Stop();
        worker.Stop();
    } catch (const std::exception& e) {
