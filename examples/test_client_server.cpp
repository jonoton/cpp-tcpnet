...
void RunClient() {
    ...
    // Wait for connection to actually establish before sending (with a timeout of 10 seconds)
    ...
    std::cout << "[Client] Data sent to buffer. Waiting to flush and receive echo..." << std::endl;
...
int main() {
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "%.2f %s", scaled.value, scaled.unit);
    std::cout << "Test completed. Total bytes received by server: " << bytes_received.load() << " (" << buf << ")" << std::endl;
    return 0;
}
