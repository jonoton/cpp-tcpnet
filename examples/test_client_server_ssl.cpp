...
void RunServer() {
  ...
  // Verify cert files exist before starting SSL to prevent silent failures
  {
    ...
    if (!cert_file.good() || !key_file.good()) {
      std::cerr << "[Server Error] Certificate or private key file not found "
                   "at " CPPTCPNET_CERTS_DIR
                << std::endl;
      return;
    }
  }
  ...
  try {
    ...
    std::cerr << "[Server Error] Failed to enable SSL: " << e.what()
              << std::endl;
    return;
  }
#endif
...
void RunClient() {
  ...
  try {
    ...
    std::cerr << "[Client Error] Failed to enable SSL: " << e.what()
              << std::endl;
    return;
  }
#endif
  ...
  while (!client_connected) {
    if (std::chrono::steady_clock::now() - start_time >
      ...
      std::cerr << "[Client Error] Connection / SSL Handshake timeout!"
                << std::endl;
      client.Stop();
      worker.Stop();
      return;
  ...
  std::cout << "[Client] Secure data sent to buffer. Waiting to flush and "
               "receive echo..."
            << std::endl;
...
int main() {
  ...
  std::cout << "SSL/TLS support is NOT enabled in the library build. Skipping "
               "SSL test client/server example."
            << std::endl;
  return 0;
#endif

  SetLogger([](LogSeverity severity, const std::string& cls,
  ...
  char buf[128];
  snprintf(buf, sizeof(buf), "%.2f %s", scaled.value, scaled.unit);
  std::cout << "Secure Test completed. Total bytes received by server: "
