...
int main() {
    ...
    worker.AddSubscription<cpptcpnet::ConnectionEvent>(sub, [](const cpptcpnet::ConnectionEvent& event) {
        if (event.state == cpptcpnet::ConnectionState::Connected) {
            std::cout << "Connected to server over SSL/TLS: " << event.session_id << std::endl;
        } else {
            std::cout << "Disconnected from server: " << event.session_id << std::endl;
        }
    });
    auto err_sub = client.GetEventBroker().Subscribe<cpptcpnet::ErrorEvent>("error_events");
