...
namespace cpptcpnet
{
    ...
    /**
     * @brief A structure representing a scaled numeric value and its corresponding unit label.
     */
    struct ScaledUnit
    {
        double value;
    ...
    /**
     * @brief Scales a byte count to its most appropriate binary unit (B, KB, MB, GB, TB).
    ...
    /**
     * @brief Scales a bit rate (bits per second) to its most appropriate decimal unit (bps, Kbps, Mbps, Gbps, Tbps).
     ...
     */
    using LogCallback = std::function<void(LogSeverity severity, const std::string &className, const std::string &message)>;
    ...
    namespace internal
    {
        ...
        public:
            ...
            bool Initialize()
            {
                ...
                // Set write_fd_ to non-blocking before connect to prevent blocking indefinitely
                u_long mode = 1;
                ...
                int res = connect(write_fd_, reinterpret_cast<sockaddr *>(&addr), sizeof(addr));
                if (res < 0)
                {
                    int err = GetLastSocketError();
                ...
                if (getsockname(write_fd_, reinterpret_cast<sockaddr *>(&local_write_addr), &local_write_len) < 0)
                {
                    CLOSE_SOCKET(write_fd_);
                    CLOSE_SOCKET(listener);
                    return false;
                ...
                // Loop accepting connections until we get the one from our own write_fd_ or timeout
                ...
                while (true)
                {
                    ...
                    if (select_res > 0)
                    {
                        ...
                        socket_t client = accept(listener, reinterpret_cast<sockaddr *>(&peer_addr), &peer_len);
     ...
     */
    inline void Log(LogSeverity severity, const std::string &className, const std::string &message)
    {
        LogCallback handler;
        {
            std::lock_guard<std::mutex> lock(internal::GetLoggerMutex());
    ...
    /**
     * @brief A strongly-typed event broadcasted over the PubSub broker when a socket changes state.
     */
    ...
    /**
     * @brief A strongly-typed event broadcasted over the PubSub broker when a background error occurs.
     */
    struct ErrorEvent
    {
        int error_code;
     ...
     */
    using ErrorHandler = std::function<void(int error_code, const std::string &message)>;
     ...
     *          It does NOT provide transport layer security (TLS/SSL). Do not use this
     *          for transmitting sensitive or confidential information over public networks
     *          without implementing an encryption layer on top of it.
     */
    class TcpListener
    {
    public:
         ...
         */
        using DataHandler = std::function<void(uint64_t session_id, const std::vector<uint8_t> &data)>;

        /**
         * @brief Constructs a new TcpListener.
         * @param port The port number to listen on.
         * @param bind_address The IP address to bind the listener to (defaults to "0.0.0.0").
         * @param max_clients The maximum number of concurrent clients allowed.
         */
        TcpListener(uint16_t port, const std::string &bind_address = "0.0.0.0", size_t max_clients = 1024)
            : port_(port), bind_address_(bind_address), max_clients_(max_clients), running_(false), server_fd_(INVALID_SOCKET)
#ifdef CPPTCPNET_SSL_SUPPORT
              ,
        ...
        {
            return std::chrono::milliseconds(ssl_handshake_timeout_ms_.load(std::memory_order_relaxed));
        }
        ...
        /**
         * @brief Sets the callback function invoked when data is received from a client.
        ...
        {
            return std::chrono::milliseconds(idle_timeout_ms_.load(std::memory_order_relaxed));
        }

        /**
         * @brief Sets the connection send/write timeout.
         * @param timeout The duration after which a blocked write is timed out and connection is dropped.
         */
        ...
        {
            return std::chrono::milliseconds(send_timeout_ms_.load(std::memory_order_relaxed));
        }

        /**
         * @brief Sets the listener file descriptor exhaustion throttling cooldown duration.
         * @param cooldown The cooldown duration.
         */
        void SetThrottleCooldown(std::chrono::milliseconds cooldown)
        {
            throttle_cooldown_ms_.store(static_cast<uint32_t>(cooldown.count()), std::memory_order_relaxed);
        }

        /**
         * @brief Gets the listener file descriptor exhaustion throttling cooldown duration.
         * @return The cooldown duration.
         */
        std::chrono::milliseconds GetThrottleCooldown() const
        {
            return std::chrono::milliseconds(throttle_cooldown_ms_.load(std::memory_order_relaxed));
        }
         ...
         */
        cpppubsub::PubSub &GetEventBroker()
        {
            return broker_;
        }
        ...
        /**
         * @brief Starts the server listening for incoming connections on a background thread.
         * @throws std::runtime_error If the server is already running or address is invalid.
        ...
        void Start()
        {
            ...
            if (ssl_enabled_)
            {
                ...
                {
                    if (SSL_CTX_load_verify_locations(ssl_ctx_, ssl_config_.ca_file.c_str(), nullptr) <= 0)
                    {
                        SSL_CTX_free(ssl_ctx_);
                        ssl_ctx_ = nullptr;
                        throw std::runtime_error("Failed to load CA file: " + ssl_config_.ca_file);
                    }
                    STACK_OF(X509_NAME) *calist = SSL_load_client_CA_file(ssl_config_.ca_file.c_str());
                    if (calist)
                    {
                        SSL_CTX_set_client_CA_list(ssl_ctx_, calist);
                    }
                }
                if (SSL_CTX_use_certificate_chain_file(ssl_ctx_, ssl_config_.cert_file.c_str()) <= 0)
                {
                    SSL_CTX_free(ssl_ctx_);
                    ssl_ctx_ = nullptr;
                    throw std::runtime_error("Failed to load certificate file: " + ssl_config_.cert_file);
                }
                if (SSL_CTX_use_PrivateKey_file(ssl_ctx_, ssl_config_.key_file.c_str(), SSL_FILETYPE_PEM) <= 0)
                {
                    SSL_CTX_free(ssl_ctx_);
                    ssl_ctx_ = nullptr;
                    throw std::runtime_error("Failed to load private key file: " + ssl_config_.key_file);
                }
            ...
            if (s != 0)
            {
                std::string err_msg = "getaddrinfo failed for bind address: " + bind_address_ + " error: ";
#ifdef _WIN32
            ...
            for (rp = res; rp != nullptr; rp = rp->ai_next)
            {
                ...
                int opt = 1;
#ifdef _WIN32
                if (setsockopt(server_fd_, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, reinterpret_cast<const char *>(&opt), sizeof(opt)) < 0)
#else
                ...
                {
                    int reuse_opt = 1;
                    if (setsockopt(server_fd_, SOL_SOCKET, SO_REUSEPORT, reinterpret_cast<const char *>(&reuse_opt), sizeof(reuse_opt)) < 0)
                    {
                        last_err = GetLastSocketError();
                        CLOSE_SOCKET(server_fd_);
                        server_fd_ = INVALID_SOCKET;
                ...
                {
                    int no = 0;
                    setsockopt(server_fd_, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const char *>(&no), sizeof(no));
                }

                if (bind(server_fd_, rp->ai_addr, static_cast<int>(rp->ai_addrlen)) == 0)
                {
                    break; // Success!
                }
            ...
            if (server_fd_ == INVALID_SOCKET)
            {
                Log(LogSeverity::Error, "TcpListener", "Failed to bind to " + bind_address_ + ":" + std::to_string(port_));
                throw std::system_error(last_err, std::system_category(), "Failed to bind");
            }
            ...
            if (!wakeup_channel_.Initialize())
            {
                CLOSE_SOCKET(server_fd_);
                server_fd_ = INVALID_SOCKET;
                Log(LogSeverity::Error, "TcpListener", "Failed to initialize WakeUpChannel");
            ...
            catch (...)
            {
                ...
                Log(LogSeverity::Error, "TcpListener", "Failed to start poll loop thread.");
                throw;
            }

            Log(LogSeverity::Info, "TcpListener", "Started on port " + std::to_string(port_));
        }
    ...
    private:
        ...
        std::unordered_map<socket_t, std::chrono::steady_clock::time_point> ssl_handshake_start_times_;
        ...
        std::unordered_map<socket_t, std::chrono::steady_clock::time_point> last_activity_time_;
        std::unordered_map<socket_t, std::chrono::steady_clock::time_point> last_write_time_;
        ...
        // SSL_read returning WANT_WRITE means renegotiation (wait for POLLOUT to retry read).
        // SSL_write returning WANT_READ means renegotiation (wait for POLLIN to retry write).
        // SSL_write returning WANT_WRITE is normal (wait for POLLOUT to retry write).
        ...
        void ReportError(int error_code, const std::string &message)
        {
            ...
            if (err_handler && worker_pool_)
            {
                (void)worker_pool_->Enqueue([err_handler, error_code, message]()
                    ...
                    {
                        Log(LogSeverity::Error, "TcpListener", "Exception in user error handler: " + std::string(e.what()));
                    }
                    catch (...)
                    {
                        Log(LogSeverity::Error, "TcpListener", "Unknown exception in user error handler");
                    } });
            }
        ...
        void RunPollLoop()
        {
            ...
            while (running_)
            {
                ...
                if (listener_throttled_)
                {
                    ...
                    {
                        auto remaining = std::chrono::duration_cast<std::chrono::milliseconds>(throttle_until_ - now).count();
                ...
                std::vector<socket_t> ssl_handshake_timeout_fds;
                {
                    std::lock_guard<std::mutex> lock(poll_mutex_);
                    auto now = std::chrono::steady_clock::now();
                    auto ssl_timeout_duration = std::chrono::milliseconds(ssl_handshake_timeout_ms_.load(std::memory_order_relaxed));
                    if (ssl_timeout_duration.count() > 0)
                    {
                        for (auto fd : ssl_handshaking_fds_)
                        {
                            auto it = ssl_handshake_start_times_.find(fd);
                            if (it != ssl_handshake_start_times_.end() && now - it->second > ssl_timeout_duration)
                            {
                ...
                {
                    Log(LogSeverity::Warning, "TcpListener", "Closing connection on fd " + std::to_string(client_fd) + " due to SSL handshake timeout.");
                    DisconnectClient(client_fd);
                }
#endif
                ...
                {
                    Log(LogSeverity::Warning, "TcpListener", "Closing idle connection on fd " + std::to_string(client_fd));
                    DisconnectClient(client_fd);
                }
                ...
                {
                    Log(LogSeverity::Warning, "TcpListener", "Closing connection on fd " + std::to_string(client_fd) + " due to send timeout.");
                    DisconnectClient(client_fd);
                }
                ...
                {
#ifdef _WIN32
                    poll_count = POLL_FUNC(poll_fds_copy.data(), static_cast<ULONG>(poll_fds_copy.size()), timeout_ms);
#else
                    poll_count = POLL_FUNC(poll_fds_copy.data(), poll_fds_copy.size(), timeout_ms);
#endif
                } while (poll_count < 0 && GetLastSocketError() == INTR_ERROR && running_);
                ...
                for (size_t i = 0; i < poll_fds_copy.size(); ++i)
                {
                    if (poll_fds_copy[i].revents & (POLLIN | POLLOUT | POLLERR | POLLHUP))
                    {
                        ...
                        {
                            if (poll_fds_copy[i].revents & POLLIN)
                                AcceptConnection();
                        }
                        else if (poll_fds_copy[i].fd == wakeup_channel_.ReadFd())
                        {
                            if (poll_fds_copy[i].revents & POLLIN)
                                wakeup_channel_.Clear();
                        }
                        else
                        {
                            ...
                            if (expected_session_id != 0)
                            {
                                bool session_valid = false;
                                {
                                    ...
                                    if (it != fd_to_session_.end() && it->second == expected_session_id)
                                    {
                                        session_valid = true;
                                    }
                                }
            ...
            while (running_)
            {
                ...
                if (client_fd == INVALID_SOCKET)
                {
...
#endif
                    {
                        ...
                        throttle_until_ = std::chrono::steady_clock::now() + std::chrono::milliseconds(throttle_cooldown_ms_.load(std::memory_order_relaxed));
                        std::lock_guard<std::mutex> lock(poll_mutex_);
                        for (auto &pfd : poll_fds_)
                        {
                            if (pfd.fd == server_fd_)
                        ...
                        ReportError(err, "Accept failed due to resource limit, throttling listener.");
                        break;
                    }
                ...
                {
                    std::lock_guard<std::mutex> lock(poll_mutex_);
                    // poll_fds_ contains server_fd_ and wakeup_fd, so number of current clients is poll_fds_.size() - 2
                    if (poll_fds_.size() - 2 >= max_clients_)
                    {
                        CLOSE_SOCKET(client_fd);
                        Log(LogSeverity::Warning, "TcpListener", "Max clients reached. Connection rejected.");
                        continue;
                    }
                    ...
                    {
                        CLOSE_SOCKET(client_fd);
                        Log(LogSeverity::Warning, "TcpListener", "Failed to set client non-blocking. Connection rejected.");
                        continue;
                    }
                    ...
                    if (ssl_enabled_)
                    {
                        ...
                        if (!ssl)
                        {
                            ...
                            CLOSE_SOCKET(client_fd);
                            Log(LogSeverity::Warning, "TcpListener", "Failed to create SSL object.");
                            continue;
                        }
                        ...
                        ssl_handshake_start_times_[client_fd] = std::chrono::steady_clock::now();
                ...
                {
                    broker_.Publish("state_events", ConnectionEvent{ConnectionState::Connected, session_id});
                }
#else
                if (session_id != 0)
                {
...
#ifdef CPPTCPNET_SSL_SUPPORT
        void HandleSslHandshake(socket_t client_fd, uint64_t session_id, bool &disconnect)
        {
            SSL *ssl = nullptr;
            {
                std::lock_guard<std::mutex> lock(poll_mutex_);
            ...
            if (ret <= 0)
            {
                ...
                else
                {
                    ...
                    Log(LogSeverity::Warning, "TcpListener", "SSL handshake failed for fd " + std::to_string(client_fd) + ": " + errbuf);
                }
            }
            else
            {
                ...
                Log(LogSeverity::Info, "TcpListener", "SSL Handshake completed for session: " + std::to_string(session_id));
                broker_.Publish("state_events", ConnectionEvent{ConnectionState::Connected, session_id});
        ...
        void ProcessClientEvents(const pollfd_t &pfd)
        {
            ...
            if (ssl_enabled_ && ssl)
            {
                ...
                {
                    std::lock_guard<std::mutex> lock(poll_mutex_);
                    // SSL_read needs POLLOUT only if it previously returned WANT_WRITE (renegotiation)
                    bool read_wants_write = ssl_read_wants_write_.find(client_fd) != ssl_read_wants_write_.end();
                    // SSL_write needs POLLIN only if it previously returned WANT_READ (renegotiation)
                    bool write_wants_read = ssl_write_wants_read_.find(client_fd) != ssl_write_wants_read_.end();

                    // Read when: POLLIN fires (normal) OR POLLOUT fires and SSL_read needs it (renegotiation)
                    ...
                    // Write when: POLLOUT fires (normal) OR POLLIN fires and SSL_write needs it (renegotiation)
                ...
                if (should_read)
                {
                    ...
                    while (read_more && !disconnect && running_)
                    {
                        ...
                        if (bytes_read > 0)
                        {
                            ...
                            broker_.Publish("transfer_events", TransferEvent{session_id, static_cast<size_t>(bytes_read), false});
                            ...
                            if (handler && worker_pool_)
                            {
                                (void)worker_pool_->Enqueue([session_id, payload = std::move(payload), handler = std::move(handler)]()
                                     ...
                                     {
                                         Log(LogSeverity::Error, "TcpListener", "Exception in user data handler: " + std::string(e.what()));
                                     }
                                     catch (...)
                                     {
                                         Log(LogSeverity::Error, "TcpListener", "Unknown exception in user data handler");
                                     } });
                            }
                        }
                ...
                if (!disconnect)
                {
                    ...
                    for (auto &master_pfd : poll_fds_)
                    {
                        if (master_pfd.fd == client_fd)
                        {
                            ...
                            // Need POLLIN if: SSL_read wants it (normal), or SSL_write needs POLLIN (renegotiation),
                            // or no special SSL state (default: always listen for incoming data)
                            bool need_pollin = ssl_read_wants_read_.find(client_fd) != ssl_read_wants_read_.end() || ssl_write_wants_read_.find(client_fd) != ssl_write_wants_read_.end() || (ssl_read_wants_write_.find(client_fd) == ssl_read_wants_write_.end());
                            if (need_pollin)
                            {
                                events |= POLLIN;
                            }
                            // Need POLLOUT if: have data to send, or SSL_read needs POLLOUT (renegotiation),
                            // or SSL_write needs POLLOUT (normal backpressure)
                            if (has_outbound || ssl_read_wants_write_.find(client_fd) != ssl_read_wants_write_.end() || ssl_write_wants_write_.find(client_fd) != ssl_write_wants_write_.end())
                            {
                                events |= POLLOUT;
                            }
            ...
            if (pfd.revents & POLLIN)
            {
                ...
                do
                {
                    bytes_read = recv(client_fd, buffer, static_cast<socket_buf_size_t>(buf_size), 0);
                } while (bytes_read < 0 && GetLastSocketError() == INTR_ERROR && running_);
                ...
                {
                    if (int err = GetLastSocketError(); err != IN_PROGRESS_ERROR && err != WOULD_BLOCK_ERROR)
                    {
                        disconnect = true;
                    }
                }
        ...
        void DisconnectClient(socket_t client_fd)
        {
            ...
            if (session_id != 0)
            {
                broker_.Publish("state_events", ConnectionEvent{ConnectionState::Disconnected, session_id});
            }
        }
    };
    ...
    public:
        ...
        /**
         * @brief Sets the callback function invoked when data is received from a server.
         * @param handler The data handler function.
         */
        void SetDataHandler(DataHandler handler)
        {
    ...
    public:
        uint64_t Connect(const std::string &ip_address, uint16_t port)
        {
            ...
            if (fd == INVALID_SOCKET)
            {
                Log(LogSeverity::Error, "TcpClient", "Failed to connect to: " + ip_address);
                throw std::system_error(last_err, std::system_category(), "Failed to connect");
            }
        ...
        ClientStats GetStats() const
        {
            ...
            {
                std::lock_guard<std::mutex> lock(poll_mutex_);
                stats.is_connected = !fd_to_session_.empty() && connecting_sockets_.empty();
            }
            return stats;
        }
         ...
         * @param session_id The unique session ID of the server connection.
         * @return True if connection was found and disconnect initiated, false otherwise.
         */
        bool Disconnect(uint64_t session_id)
        {
            socket_t fd = INVALID_SOCKET;
         ...
         */
        void SetAutoReconnect(bool enabled, std::chrono::milliseconds initial_delay = std::chrono::milliseconds(1000), std::chrono::milliseconds max_delay = std::chrono::milliseconds(30000))
        {
    ...
    private:
        struct Target
        {
            std::string ip;
            uint16_t port;
            bool operator==(const Target &o) const { return ip == o.ip && port == o.port; }
        };
        struct TargetHash
        {
        ...
        void StartInternal()
        {
            ...
            if (ssl_enabled_)
            {
                ...
                if (ssl_config_.verify_peer)
                {
                    ...
                    {
                        const char *ca_file = ssl_config_.ca_file.empty() ? nullptr : ssl_config_.ca_file.c_str();
                        const char *ca_path = ssl_config_.ca_path.empty() ? nullptr : ssl_config_.ca_path.c_str();
                ...
                if (!ssl_config_.client_cert_file.empty() && !ssl_config_.client_key_file.empty())
                {
                    if (SSL_CTX_use_certificate_chain_file(ssl_ctx_, ssl_config_.client_cert_file.c_str()) <= 0)
                    {
                        SSL_CTX_free(ssl_ctx_);
                        ssl_ctx_ = nullptr;
                        throw std::runtime_error("Failed to load client certificate: " + ssl_config_.client_cert_file);
                    }
                    if (SSL_CTX_use_PrivateKey_file(ssl_ctx_, ssl_config_.client_key_file.c_str(), SSL_FILETYPE_PEM) <= 0)
                    {
                        SSL_CTX_free(ssl_ctx_);
                        ssl_ctx_ = nullptr;
                        throw std::runtime_error("Failed to load client private key: " + ssl_config_.client_key_file);
                    }
                    if (SSL_CTX_check_private_key(ssl_ctx_) <= 0)
                    {
                        SSL_CTX_free(ssl_ctx_);
            ...
            if (!wakeup_channel_.Initialize())
            {
                Log(LogSeverity::Error, "TcpClient", "Failed to initialize WakeUpChannel");
                throw std::runtime_error("Failed to initialize WakeUpChannel");
            }
            size_t threads = worker_thread_count_.load(std::memory_order_relaxed);
            if (threads == 0)
        ...
        void ReportError(int error_code, const std::string &message)
        {
            ...
            if (err_handler && worker_pool_)
            {
                (void)worker_pool_->Enqueue([err_handler, error_code, message]()
                    ...
                    {
                        Log(LogSeverity::Error, "TcpClient", "Exception in user error handler: " + std::string(e.what()));
                    }
                    catch (...)
                    {
                        Log(LogSeverity::Error, "TcpClient", "Unknown exception in user error handler");
                    } });
            }
            broker_.Publish("error_events", ErrorEvent{error_code, message});
        }
        ...
        void RunPollLoop()
        {
            ...
            while (running_)
            {
                std::vector<pollfd_t> poll_fds_copy;
                {
                    ...
                    cv_.wait(lock, [this]()
                             { return poll_fds_.size() > 1 || !running_; });
                    if (!running_)
                        break;
                    poll_fds_copy = poll_fds_;
                }
                ...
                std::vector<socket_t> timed_out;
                {
                    std::lock_guard<std::mutex> lock(poll_mutex_);
                    auto now = std::chrono::steady_clock::now();
                    auto connect_timeout_duration = std::chrono::milliseconds(connect_timeout_ms_.load(std::memory_order_relaxed));
                    for (auto s_fd : connecting_sockets_)
                    {
                        auto it = connection_start_times_.find(s_fd);
                        if (it != connection_start_times_.end() && (now - it->second) > connect_timeout_duration)
                        {
                ...
                {
                    Log(LogSeverity::Warning, "TcpClient", "Closing connection on fd " + std::to_string(fd) + " due to SSL handshake timeout.");
                    ReportError(TIMEOUT_ERROR_CODE, "SSL handshake timed out asynchronously.");
                    DisconnectServer(fd);
                }
#endif
                ...
                for (auto client_fd : idle_connections)
                {
                    Log(LogSeverity::Warning, "TcpClient", "Closing idle connection on fd " + std::to_string(client_fd));
                    ReportError(TIMEOUT_ERROR_CODE, "Connection timed out due to idle timeout.");
                ...
                {
#ifdef _WIN32
                    poll_count = POLL_FUNC(poll_fds_copy.data(), static_cast<ULONG>(poll_fds_copy.size()), 1000);
#else
                    poll_count = POLL_FUNC(poll_fds_copy.data(), poll_fds_copy.size(), 1000);
#endif
                } while (poll_count < 0 && GetLastSocketError() == INTR_ERROR && running_);
        ...
        void HandleSslHandshake(socket_t fd, uint64_t session_id, bool &disconnect)
        {
            ...
            if (ret <= 0)
            {
                ...
                else
                {
                    ...
                    char errbuf[256];
                    ERR_error_string_n(oscode, errbuf, sizeof(errbuf));
                    Log(LogSeverity::Warning, "TcpClient", "SSL handshake failed: " + std::string(errbuf));
                }
            }
            else
            {
                ...
                Log(LogSeverity::Info, "TcpClient", "SSL Handshake completed for session: " + std::to_string(session_id));
                broker_.Publish("state_events", ConnectionEvent{ConnectionState::Connected, session_id});
        ...
        void ProcessServerEvents(const pollfd_t &pfd)
        {
            ...
            if (is_connecting)
            {
                ...
                // Portable reinterpret_cast to char* is used here to support both POSIX and Windows getsockopt() signatures.
                ...
                else
                {
                    ...
                    {
                        {
                            ...
                            for (auto &master_pfd : poll_fds_)
                            {
                                if (master_pfd.fd == fd)
                                {
                                    master_pfd.events = POLLIN; // Remove POLLOUT until Send is called
                                    break;
                                }
                            }
                        }
                    ...
                    // Proceed to handle existing POLLOUT if data was buffered while connecting
                    if (!disconnect)
                    {
                        bool needs_pollout = false;
                        {
            ...
            else
            {
                ...
                if (ssl_enabled_ && ssl)
                {
                    ...
                    {
                        std::lock_guard<std::mutex> lock(poll_mutex_);
                        bool read_wants_write = ssl_read_wants_write_.find(fd) != ssl_read_wants_write_.end();
                        bool write_wants_read = ssl_write_wants_read_.find(fd) != ssl_write_wants_read_.end();
                            ...
                            if (bytes_read > 0)
                            {
                                ...
                                if (handler && worker_pool_)
                                {
                                    (void)worker_pool_->Enqueue([session_id, payload = std::move(payload), handler = std::move(handler)]()
                                         ...
                                         {
                                             Log(LogSeverity::Error, "TcpClient", "Exception in user data handler: " + std::string(e.what()));
                                         }
                                         catch (...)
                                         {
                                             Log(LogSeverity::Error, "TcpClient", "Unknown exception in user data handler");
                                         } });
                                }
                            }
                            else if (bytes_read == 0)
                    ...
                    if (!disconnect)
                    {
                        ...
                        for (auto &master_pfd : poll_fds_)
                        {
                            if (master_pfd.fd == fd)
                            {
                                ...
                                bool need_pollin = ssl_read_wants_read_.find(fd) != ssl_read_wants_read_.end() || ssl_write_wants_read_.find(fd) != ssl_write_wants_read_.end() || (ssl_read_wants_write_.find(fd) == ssl_read_wants_write_.end());
                                if (need_pollin)
                                {
                                    events |= POLLIN;
                                }
                                if (has_outbound || ssl_read_wants_write_.find(fd) != ssl_read_wants_write_.end() || ssl_write_wants_write_.find(fd) != ssl_write_wants_write_.end())
                                {
                                    events |= POLLOUT;
                                }
                                master_pfd.events = events;
                ...
                if (pfd.revents & POLLIN)
                {
                    ...
                    do
                    {
                        bytes_read = recv(fd, buffer, static_cast<socket_buf_size_t>(buf_size), 0);
                    } while (bytes_read < 0 && GetLastSocketError() == INTR_ERROR && running_);
        ...
        void DisconnectServer(socket_t fd)
        {
            ...
            if (has_target && auto_reconnect_enabled_.load(std::memory_order_relaxed) && running_)
            {
                std::lock_guard<std::mutex> lock(poll_mutex_);
                if (reconnecting_targets_.find(target) == reconnecting_targets_.end())
                {
                    ...
                    info->thread = std::thread([this, target, info]()
                        ...
                        while (running_) {
                            ...
                            while (running_) {
                                if (std::chrono::steady_clock::now() - sleep_start >= current_delay) {
                                    break;
                                }
                            ...
                            try {
                                ...
                                Log(LogSeverity::Warning, "TcpClient", "Auto-reconnect Connect call failed: " + std::string(e.what()));
                            }
                            ...
                            if (connect_initiated) {
                                ...
                                while (running_ && !resolved) {
                                    ...
                                    if (it == session_to_fd_.end()) {
                                        ...
                                        if (connecting_sockets_.find(new_fd) == connecting_sockets_.end()) {
                                            resolved = true;
                                            success = true;
                                        }
                                    }
                                ...
                                if (success) {
                                    Log(LogSeverity::Info, "TcpClient", "Auto-reconnect succeeded for target " + target.ip + ":" + std::to_string(target.port));
                                    std::lock_guard<std::mutex> lock_state(poll_mutex_);
    ...
    /**
     * @brief Utility class to track sliding-window network throughput using PubSub TransferEvents.
     */
    ...
    public:
         ...
         * @param window_duration The sliding-window duration over which to compute throughput.
         */
        ThroughputTracker(cpppubsub::PubSub &broker, std::chrono::milliseconds window_duration = std::chrono::seconds(1))
            : window_duration_(window_duration), total_send_bytes_(0), total_recv_bytes_(0)
        {
            sub_ = broker.Subscribe<TransferEvent>("transfer_events", 10000, cpppubsub::OverflowPolicy::DropOldest);
            worker_.AddSubscription<TransferEvent>(sub_, [this](const TransferEvent &event)
                                                   {
         ...
         */
        double GetSendThroughputBytesPerSec()
        {
            return CalculateThroughput(true);
        }
         ...
         */
        double GetRecvThroughputBytesPerSec()
        {
            return CalculateThroughput(false);
        }
    ...
    private:
        ...
        double CalculateThroughput(bool is_send)
        {
            ...
            double seconds = std::chrono::duration_cast<std::chrono::duration<double>>(window_duration_).count();
